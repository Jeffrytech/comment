import logo from "../../logo.svg";
import { processDate } from "../../utils";
import { loggedInUser } from "../../data";
import FormInput from "../FormInput/form_input";
import Notification from "../Notification/Notification";
import Backdrop from "../Backdrop/Backdrop";
import RepliesToReply from "./RepliesToReply";

import { useState, useEffect } from "react";

function CommentReplies({
  data,
  handleReplyToReplySubmit,
  grandParentId,
  handleInnerLike,
  fetchReplies,
}) {
  const [showReplyInput, setShowReplyInput] = useState(false);
  const [comment, setComment] = useState("");
  const [postingComment, setPostingComment] = useState(false);
  const [showMessage, setShowMessage] = useState(false);
  const [hasLiked, setHasLiked] = useState(false);
  const [repliesToShow, setRepliesToShow] = useState([]);
  const [showMoreButton, setShowMoreButton] = useState(true);
  const [fetchingReply, setFetchingReply] = useState(false);

  useEffect(() => {
    const userLikeId =
      data.likes &&
      data.likes.findIndex((like) => like.userId === loggedInUser.id);
    if (userLikeId !== -1) {
      setHasLiked(true);
    } else {
      setHasLiked(false);
    }

    //For the paging
    if (data.replies && data.replies.length > 2) {
      setRepliesToShow(data.replies.slice(0, 2));
    } else {
      setRepliesToShow(data.replies);
    }
  }, [data]);

  async function localHandleReply(
    event,
    comment,
    publishToProfile,
    parentId,
    grandParentId
  ) {
    setPostingComment(true);

    await handleReplyToReplySubmit({
      event,
      comment,
      publishToProfile,
      parentId,
      grandParentId,
    });
    setPostingComment(false);
    setShowReplyInput(false);

    //For the paging
    if (data.replies && data.replies.length > 2) {
      setRepliesToShow(data.replies.slice(0, 2));
    } else {
      setRepliesToShow(data.replies);
    }
    setShowMoreButton(true);
  }

  function handleShowReplyInput() {
    const registeredDays =
      (new Date().getTime() -
        new Date(loggedInUser.user_created_at).getTime()) /
      (1000 * 60 * 60 * 24);
    if (registeredDays < 7) {
      setShowMessage(true);
    } else {
      setShowReplyInput(!showReplyInput);
    }
  }

  function removeNotification() {
    setShowMessage(false);
  }

  function localHandleLike(id, grandParentId, hasLiked) {
    const prevlikeState = hasLiked;
    setHasLiked(!hasLiked);
    handleInnerLike(id, grandParentId, prevlikeState);
  }

  function handleShowMore() {
    setRepliesToShow(data.replies);
    setShowMoreButton(false);
  }

  async function localShowReplies(id, parentId) {
    setFetchingReply(true);
    await fetchReplies(id, parentId);
    setFetchingReply(false);
    //For the paging
    if (data.replies && data.replies.length > 2) {
      setRepliesToShow(data.replies.slice(0, 2));
    } else {
      setRepliesToShow(data.replies);
    }
  }

  function cancel() {
    setShowReplyInput(false);
  }

  let repliesLength = 0;
  repliesLength = data.replies && data.replies.length;

  return (
    <div className="replies" style={{ overflowY: "scroll", height: "auto" }}>
      {showMessage && (
        <div>
          <Backdrop backdropClicked={removeNotification} show={showMessage} />
          <Notification
            handleOkClicked={removeNotification}
            title="Not qualifed"
            message="Account is less than 7 days and cannot reply to a comment at this time"
          />
        </div>
      )}

      <div>
        <div className="top">
          <div className="profile-set">
            <div className="d-flex">
              <div className="profile">
                <img src={logo} />
              </div>
              <div className="d-flex">
                <div>
                  <span>{data && data.author && data.author.firstName}</span>
                  <br />
                  <span>{processDate(data.createdAt)}</span>
                </div>

                {data.author && data.author.id === loggedInUser.id && (
                  <span
                    style={{
                      background: "green",
                      height: "3vh",
                      padding: "3px",
                      color: "white",
                      borderRadius: "5px",
                    }}
                  >
                    {" "}
                    author{" "}
                  </span>
                )}
              </div>
            </div>

            <div className="dropdowndot">
              <i className="fa-solid fa-ellipsis dropbtndot"></i>
              <span className="dropdown-contentdot" id="vb">
                Report this response
              </span>
            </div>
          </div>
        </div>
        <br />
        <p>{data && data.comment}</p>
        <div className="replies-actions">
          <div
            className="reply-container"
            onClick={() => localHandleLike(data.id, grandParentId, hasLiked)}
          >
            <i
              style={{ color: hasLiked ? "red" : null }}
              className="fa-regular fa-heart m-1"
            >
              {" "}
            </i>
            <i className="m-1 fa-solid fa-up-long"></i>
            <span>{data && data.likes && data.likes.length}</span>
          </div>
          <div
            className="reply-container"
            onClick={() => localShowReplies(data.id, grandParentId)}
          >
            <i className="fas fa-comment"></i>{" "}
            {fetchingReply ? (
              <span>fetching replies...</span>
            ) : (
              <span>{data.noOfComments} replies</span>
            )}
          </div>
          <div onClick={handleShowReplyInput} className="reply-container">
            <span>Reply</span>
          </div>
        </div>
        {showReplyInput && (
          <FormInput
            postingComment={postingComment}
            handleSubmit={localHandleReply}
            comment={comment}
            setComment={setComment}
            parentId={data.id}
            grandParentId={grandParentId}
            cancel={cancel}
          />
        )}
      </div>
      {repliesToShow &&
        repliesToShow.map((reply2, i) => {
          return (
            <div key={i}>
              <RepliesToReply data={reply2} />
            </div>
          );
        })}
      {repliesLength > 2 && showMoreButton && (
        <span onClick={handleShowMore} className="reply-container text-primary">
          Show {repliesLength - 2} more{" "}
          {repliesLength > 3 ? "replies" : "reply"}
        </span>
      )}
    </div>
  );
}

export default CommentReplies;
